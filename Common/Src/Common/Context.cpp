...
namespace Cm
{
    Context::Context(const std::string_view& title, WindowFlags flags)
        : m_Window(title, flags), m_ImGuiEnabled(false), m_DeltaTime(0), m_ClearColor(DefaultClearColor), m_ClearOptions(0)
    {
    }

    Context::Context(const std::string_view& title, int width, int height, WindowFlags flags)
        : m_Window(title, width, height, flags), m_ImGuiEnabled(false), m_DeltaTime(0), m_ClearColor(DefaultClearColor), m_ClearOptions(0)
    {
    }
    ...
    void Context::SetClearOptions(int clearOptions)
    {
        m_ClearOptions = clearOptions;
    }
    ...
    {
        ImGuiIO& io = ImGui::GetIO();
        io.Fonts->AddFontFromFileTTF(RESOURCE_DIR "/Arimo/ArimoNerdFont-Regular.ttf", 16.0f);
